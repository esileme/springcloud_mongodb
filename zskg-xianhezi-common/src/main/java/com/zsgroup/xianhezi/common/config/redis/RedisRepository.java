...
public class RedisRepository {
    ...
    public Map<String,Long> keyExpire(final String key) {
        final Map<String,Long> keysMap = new HashMap<>();
        redisTemplate.execute((RedisCallback<Map<String,Long>>) connection -> {
            Set<String> keys = redisTemplate.keys(key + "*");
            for (String key1 : keys) {
                Long ttl = connection.ttl(key1.getBytes(DEFAULT_CHARSET));
                keysMap.put(key1,ttl);
            }
            return keysMap;
        });
        return keysMap;
    ...
    /**
     * 在redis中每次取出值，将value加一
     * @param key
     * @return
     */
    public String getOrderNo(final String key) {
        String resultStr = redisTemplate.execute((RedisCallback<String>) connection -> {
            RedisSerializer<String> serializer = getRedisSerializer();
            byte[] keys = serializer.serialize(key);
            byte[] values = connection.get(keys);
            connection.incr(keys);
            return serializer.deserialize(values);
        });
        LOGGER.info("[redisTemplate redis]取出 缓存  url:{} ", key);
        return resultStr;
